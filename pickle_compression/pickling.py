...
def _unpack_compression_args(
    ...
    if compression is not None:
        if isinstance(compression, str):
            ...
            return compression["method"], {k: compression[k] for k in compression.keys() if k != "method"}
...
def load_compressed(path: Union[str, pathlib.Path], compression: Optional[Union[str, dict]] = None) -> Any:
    """
...
def get_pickled_size(
    ...
    :param obj: the object to pickle
    :param compression: the compression method used. Either a string or a dict with key 'method' set to the compression
                        method and other key-value pairs are forwarded to compress() of the compression library.
                        Inspired by the pandas.to_csv interface.
    :param dump_function: the function being called to dump the object, can be a custom pickler.
                          Defaults to pickle.dump()
    """
